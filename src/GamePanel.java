...
public class GamePanel extends JFrame implements KeyListener {
    ...
    GamePanel() {
        ...
        expertButton.setBounds(360, 320, 80, 25);
        ...
        expertButton.addActionListener(actionListener);
        ...
        start.addActionListener(e -> {
            ...
            GamePanel.this.remove(expertButton);
        ...
        ImageIcon rocket1 = new ImageIcon(getClass().getResource("/rocket.png"));
    ...
    private void restartGame() {
        if (playAgain == JOptionPane.OK_OPTION) {
            ...
            for (Obstacle o : obstacles) {
                o.stop(); // stop timers of old obstacles
                this.remove(o.getLabel());
            }
            obstacles.clear();
    ...
    public void inFrame(){
        ...
        int y = label.getY();
        if (x < 0) x=0;
        else if (x +label.getWidth() > this.getWidth()) x = this.getWidth() - label.getWidth();
        if (y < 0) y=0;
        else if (y +label.getHeight() > this.getHeight()) y = this.getHeight() - label.getHeight();
        label.setLocation(x, y);
    }

    public void move() {
        if (w && d) label.setLocation(label.getX() + 7, label.getY() - 7);
        else if (a && w) label.setLocation(label.getX() - 7, label.getY() - 7);
        else if (s && a) label.setLocation(label.getX() - 7, label.getY() + 7);
        else if (s && d) label.setLocation(label.getX() + 7, label.getY() + 7);
        else {
            if (w) label.setLocation(label.getX(), label.getY() - 7);
            if (a) label.setLocation(label.getX() - 7, label.getY());
            if (s) label.setLocation(label.getX(), label.getY() + 7);
            if (d) label.setLocation(label.getX() + 7, label.getY());
        }
    }
}
